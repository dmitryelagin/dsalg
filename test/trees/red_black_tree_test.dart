...
void main() {
  ...
  group('RedBlackTree', () {
    testBaseBinarySearchTree(<K, V>(compare, [entries]) {
      return RedBlackTree(compare, entries ?? const {});
    });
  });
  ...
  group('RedBlackTree', () {
    ...
    late RedBlackTree<int, int> tree, worstTree;

    setUp(() {
      compareInt = IntComparator();
      final firstItems = random.nextIntMap(500, absentItem);
      tree = RedBlackTree(compareInt, firstItems);
      ...
      worstTree = RedBlackTree(compareInt, worstItems.toMap());
    });
